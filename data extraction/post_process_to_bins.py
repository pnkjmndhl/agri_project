...
#filenames = ["mode_split_pred1_0_1","mode_split_pred1_0_2"]
filenames = ["all_cws_modeshift_int_1", "all_cws_modeshift_int_2"]

for filename in filenames:
    data = pd.read_csv( filename + ".csv")
    data = data[(data.usda == 1) |(data.usda == 2)]
    #all_commodities = list(data.commodty.unique())
    data['dist_bin'] = data[google_maps_dist].map(get_dist_bin)
    #work on this later
    keep_columns = ['Unnamed: 0', commodity, google_maps_dist, use_rate, 'dist_bin', 'lost_rev', 'lost_ton'] #note this
    ...
    use_rate_df = use_rate_df[use_rate_df[use_rate] != 0]
